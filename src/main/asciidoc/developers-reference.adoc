...
* `waitingFor(..)` - defines a timeout _conditional_. The built query will
  evaluate as *successful* if _any_ responses are consumed within the
  (approximate) given time limit. There are a few different methods declared,
...
* `orEmpty()` - defines the query to return an empty `Collection` in case the
  _conditionals_ do not evaluate to *successful*.evaluated
