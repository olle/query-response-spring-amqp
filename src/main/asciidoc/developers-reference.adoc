...
blocking _conditional_. We'll explain more about the possible conditional calls
in the table below. In the example above, the call to `waitingFor(..)` defines
that the call will block for around 800 milliseconds.
