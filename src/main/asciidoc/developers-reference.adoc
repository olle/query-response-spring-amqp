...
Queries are built and published using the `QueryBuilder.queryFor(..)` method.
Any of the terminal calls to either `orEmpty()`, `orDefaults(..)` or
...
Since the call to `orEmpty()` will block, users have to specify any of the
blocking _conditions_ - here the call to `watingFor(..)` defines that the call
will block for around 800 milliseconds.
