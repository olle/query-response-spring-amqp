...
as a dependency to provide access from methods in Spring any component.
...
In example above, the published query is defined by the **term** `"authors"`.
This is how the most basic contract of {qr-pfx} is defined. A any string or
text term may be published as a query. The second argument is the expected
type of any received response elements. It is not published with the query, but
rather used to coerce or interpret any received responses.

This means that regardless of the payload of any response {qr-name} will attempt
to read the response elements as the type `String.class`.
