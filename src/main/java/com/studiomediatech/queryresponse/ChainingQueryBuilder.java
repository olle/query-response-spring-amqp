 ...
 *
 * <p>
 * You begin building a query by calling the static method like this:
 * </p>
 *
 * <pre>
 ...
 *
 * <p>
 * A {@link ChainingQueryBuilder queryBuilder-instance} is a container for a composed or configured query. It is is much
 * like a command-pattern object, providing all the properties required in order to publish the query, await responses
 * and return the results.
 * </p>
 *
 * @param <T>
 *            the <em>coerced</em> type of the query's response element {@link Collection collection}.
 */
public final class ChainingQueryBuilder<T> {
    ...
    /**
     * The duration to block a query before returning either the gathered results, provided defaults or throw some given
     * exception. The duration is required and must be set to at least a non-negative, non-zero duration of
     * milliseconds.
     */
    ...
    /**
     * An optional consumer of an {@link Throwable throwable}, allowing clients to inspect any failures occurring during
     * a query life-cycle.
     */
     ...
     *
     * <p>
     * The actual blocking time is not exact. The current contract will ensure blocking for <em>at least</em> the amount
     * of time specified.
     * </p>
     *
     * <p>
     * Queries are always bound to a blocking timeout, and will return after at most the given duration. Combined with
     * the <em>taking...</em> conditionals ({@link #takingAtLeast(int)} or {@link #takingAtMost(int)}), this can be used
     * to either return earlier, use a {@link #orDefaults default} or {@link #orThrow(Supplier) fail}.
     * </p>
     *
     * @param millis
     *            duration to block on the calling thread. At the most {@link Long#MAX_VALUE} milliseconds is allowed.
     *
     * @return the query builder, for chaining further calls
     *
     * @throws IllegalArgumentException
     *             for any 0, negative or too long duration.
     */
     ...
     *
     * @param amount
     *            the amount of the duration to block on the calling thread. An amount representing the duration of at
     *            the most {@link Long#MAX_VALUE} milliseconds is allowed.
     * @param timeUnit
     *            the unit of the blocking duration
     *
     * @return the query builder, for chaining further calls
     *
     * @throws IllegalArgumentException
     ...
     *
     * @param duration
     *            to block on the calling thread. A duration of at the most {@link Long#MAX_VALUE} milliseconds is
     *            allowed.
     *
     * @return the query builder, for chaining further calls
     *
     ...
     *
     * <p>
     * Queries always block for a specified {@link #waitingFor(long) duration}.Combined with this specified amount of
     * maximum elements to consume, the query can be made to return earlier.
     * </p>
     *
     * @param atMost
     *            number of elements to consume, must be greater than 0.
     *
     * @return the query builder, for chaining further calls
     *
     * @throws IllegalArgumentException
     *             for any values less than 1
     */
     ...
     *
     * @param atLeast
     *            number of elements required, for the query to be successful, must be an amount greater than 0, and
     *            cannot be lower than a previous call to {@link #takingAtMost} has set as a limit
     *
     * @return the query builder, for chaining further calls
     */
     ...
     *
     * @param handler
     *            to receive the {@link Throwable error}
     *
     * @return the query builder, for chaining further calls
     */
     ...
     *
     * @param defaults
     *            collection of results to use for a failing query
     *
     * @return the query builder, for chaining further calls
     */
     ...
     *
     * @param defaults
     *            collection provider, for results to use if the query fails
     *
     * @return the query builder, for chaining further calls
     */
     ...
     *
     * @param throwable
     *            supplier, invoked to get a throwable to throw in case the query fails
     *
     * @return the query builder, for chaining further calls
     */
    ...
    /*
     * NOTE: Callers of this method are explicit, and not wrapped or overload in calls to themselves, in order to show
     * developers which methods represent terminal-state in the fluid builder API.
     *
    ...
    private void assertTakingAtMostAndAtLeast() {
        ...
        if (atLeast != 0 && atLeast > atMost && atMost != 0) {
            throw new IllegalArgumentException(
                    "Cannot require more than limit. " + "atLeast: " + atLeast + ", atMost: " + atMost);
        }
     ...
     *
     * @param sink
     *            to consume this builder, in the terminal operation call to any {@code orXYZ(..)} methods, and return
     *            the results {@code null}.
     *
